...
| `Actor` | The user who uploaded the file. `Type` is `User` for SFTP/FTPS/web portal uploads, `ShareLink` for share link uploads, `Admin` for admin users, or `System` if the uploader could not be determined. |
...
* **Actor Type**: The type of the actor - `User` for SFTP/FTPS/web portal users, `ShareLink` for share link uploads, `Admin` for admin users, or `System` if the actor could not be determined.
