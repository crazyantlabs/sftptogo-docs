...
To activate SAML SSO, turn it on in the SSO section and follow the on-screen steps to connect your provider.

SFTP To Go supports Google Workspace, Microsoft ADFS, Microsoft Entra, Okta, and custom SAML identity providers.
...
If just-in-time provisioning is not enabled, users must be invited manually before they can sign in with SAML SSO. Only accounts that already exist in your organization will be allowed to authenticate through your identity provider.

:::info
Using SAML-SSO is only available with certain plans. Read more about our different plans [here](https://sftptogo.com/pricing)
:::
