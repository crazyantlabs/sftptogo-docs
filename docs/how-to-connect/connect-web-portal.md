...
* **Search**: Type in the search bar and press Enter to find files and folders. The search starts from the folder you are in and looks through all of its subfolders — use the **Search in** control to widen it to everywhere or narrow it to the current folder only. Every word you type must appear in the name, in any order, so "annual report" finds `2026_annual-report.pdf`. Matches are case-insensitive and work in any language. Each result shows the folder it lives in, and clicking a result takes you right to it. When nothing matches your exact text, close matches are shown instead, so a small typo still finds the file you're after. Navigating to another folder leaves the search. Press `/` or `Cmd+K` to jump to the search bar.
