...
* **Search**: Type in the search bar and press Enter to find files and folders anywhere in your accessible folders — the search looks through all subfolders, not just the visible list. Every word you type must appear in the name, in any order, so "annual report" finds `2026_annual-report.pdf`. Matches are case-insensitive and work in any language. Each result shows the folder it lives in, and clicking a result takes you right to it. When nothing matches your exact text, close matches are shown instead, so a small typo still finds the file you're after. Use the **Search in** control to scope the search to a specific folder, and press `/` or `Cmd+K` to jump to the search bar.
